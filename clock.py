...
while True:
    ...
    try:
        ...
        PRCNTBLOCKED = data['ads_percentage_today']
        CLIENTS = data['unique_clients']
        TOTALBLKLIST = data['domains_being_blocked']
    except KeyError:
        time.sleep(1)
        continue
    ...
    if not buttonA.value:  # just button A pressed
        ...
        draw.text((x, y), "Ads Blocked: {}".format(str(ADSBLOCKED)), font=font2, fill="#00FF00")
        y += font.getsize(str(ADSBLOCKED))[1]
        draw.text((x, y), "Clients: {}".format(str(CLIENTS)), font=font2, fill="#0000FF")
        y += font.getsize(str(CLIENTS))[1]
        draw.text((x, y), "DNS Queries: {}".format(str(DNSQUERIES)), font=font2, fill="#FF00FF")
        y += font.getsize(str(DNSQUERIES))[1]
        draw.text((x, y), "BLK List: {}".format(str(TOTALBLKLIST)), font=font2, fill="#FF8000")
